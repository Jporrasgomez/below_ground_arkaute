...
sampling_dates <- read.csv("data/sampling_dates.csv") %>% 
  mutate(sampling = as.factor(sampling),
         ...
         day = day(date), 
         year = year(date)) %>% 
  select(sampling, date, day, month, year, one_month_window, omw_date, label_micro) %>% 
...
sampling_days_data <- data %>% 
  ...
  select(t_top, t_bottom, t_ground, vwc, date, time, plot)  
...
sampling_days_data <- sampling_days_data %>% 
  ...
  summarize(mean_t_top = mean(t_top), 
            mean_t_bottom = mean(t_bottom),
...
variables = c("t_top", "t_bottom", "t_ground", "vwc")
...
gg_boxplot_alldata <- 
  ...
  labs(y = ytitle, x = NULL, title = "All year")
  ...
  gg_boxplot_daily_average <- 
    ...
    theme_otc + 
    theme(legend.position = "none") +
    labs(y = ytitle, x = NULL, title = "All year; aggregated at daily average")
...
filter(variable == variables[i]) %>%                                       #### Modify in this line the variable or variables we want to see
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = color_no_otc, linewidth = 1) +
  geom_line(aes(group = variable), color = color_otc, linewidth = 2) +
  geom_line(aes(x = time,
                y = mean_diff_value + sd_diff_value,
                group = variable) , linetype = "dashed", color = color_otc, linewidth = 1) +
  geom_line(aes(x = time,
                y = mean_diff_value - sd_diff_value,
                group = variable) , linetype = "dashed", color = color_otc, linewidth = 1) + 
...
gg_year_diff <- 
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = color_no_otc) +
  geom_line(color = color_otc, linewidth = 0.5) +
  geom_smooth(stat = "smooth", alpha = 0.2, color = color_otc, fill = color_otc) +
  labs(
    x = NULL,
    y = "Temperature gain inside OTC (ºC)"
  ) +
...
gg_vwc_vs_t <- 
  vwc_data %>% 
  ggplot(aes(y = vwc_mean, x = t_ground_mean, color = OTC_label, fill = OTC_label)) + 
