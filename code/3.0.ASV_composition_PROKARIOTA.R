...
##source("ASV_num/1.first_script.R"); rm(list = setdiff(ls(), c("prokariota_ASV_sampling", "biomass_imp", "biomass_noimp")))
...
prokariota_ASV_raw <- read.csv("data/prokariota_ASV_raw.csv") %>%
  select(treatment, plot, sampling, ASV_num, abundance) 


#prokariota_ASV_filter <- prokariota_ASV_raw %>% 
#  filter(abundance > 0.001)


####### DIFFERENCES AT SAMPLING x TREATMENT LEVEL #############

prokariota_ASV_sampling <- prokariota_ASV_raw %>% 
...
treats <- unique(prokariota_ASV_sampling$treatment)
...
for(i in seq_along(treats)){
  ...
  list1[[i]] <- subset(prokariota_ASV_sampling, treatment == treats[i])
...
samps <- sort(unique(prokariota_ASV_sampling$sampling))

for(i in 1:length(samps)){
  
  sp_wide <- prokariota_ASV_sampling %>% 
