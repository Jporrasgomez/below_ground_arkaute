...
LRR_dynamics <- function(data, variable){
  ...
  effect <- data %>% 
    select(date,  date_label_noyear,  sampling, treatment, mean, sd) %>% 
    distinct()
  
  effect_c <- effect %>% 
    filter(treatment == "c") %>% 
    select(date, sampling, date_label_noyear, mean, sd) %>% 
    rename(mean_c = mean,
           sd_c = sd)
  
  effect_wp <- data %>% 
    filter(treatment == "wp") %>% 
    select(date, sampling, date_label_noyear, treatment, mean, sd) %>% 
    rename(mean_wp = mean,
           sd_wp = sd) %>% 
    distinct()
